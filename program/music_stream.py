...
async def play_tg_file(c: Client, m: Message, replied: Message = None, link: str = None):
    ...
    if replied.audio or replied.voice:
        if not link:
            suhu = await replied.reply("📥 s ...")
        else:
            suhu = await m.reply("📥 s ...")
...
async def audio_stream(c: Client, m: Message):
    ...
    if replied:
        ...
        if len(m.command) < 2:
            ...
            suhu = await c.send_message(chat_id, "🔍 **ss...**")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search == 0:
                await suhu.edit("❌ **  ..s **")
            ...
            if "m3u8" in url:
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await msg.delete()
                        await remove_active_chat(chat_id)
                        await m.reply_text("ss s , s     s `/vstream`  s.")
