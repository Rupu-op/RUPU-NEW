...
def gen_chlog(repo, diff):
    ...
    ch_log = ""
    tldr_log = ""
    ch = f"<b>ᴜᴘᴅᴀᴛᴇs ғᴏʀ <a href={upstream_repo_url}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"ᴜᴘᴅᴀᴛᴇs {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def update_bot(_, message: Message):
    ...
    if update_avail:
        await msg.edit("✅ ᴜᴘᴅᴀᴛᴇ ғɪɴɪsʜᴇᴅ !\n\n• ʙᴏᴛ ʀᴇsᴛᴀʀᴛᴇᴅ ..ʙᴀᴄᴋ ɪɴ ᴍᴀx 5 ᴍɪɴ.")
        system("git pull -f && pip3 install --no-cache-dir -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
    await msg.edit(f"❖ ʙᴏᴛ ɪs  **ᴜᴘ ᴛᴏ ᴅᴀᴛᴇ** ᴡɪᴛʜ [main]({UPSTREAM_REPO}/tree/main) ❖", disable_web_page_preview=True)
...
async def restart_bot(_, message: Message):
    try:
        ...
        return
    await msg.edit_text("✅ ʙᴏᴛ ʀᴇsᴛᴀʀᴛᴇᴅ!\n\n» ʙᴀᴄᴋ ɪɴ ᴍᴀx 10 ᴍɪɴ.")
